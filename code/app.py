...
# --- HELPERS: agent runners + score parser (reused by single + batch modes) ---

def build_telemetry_prompt(row_data, selected_id, selected_time, config):
    field_lines = []
...
def run_explainer_agnostic(telemetry_prompt, config):
    sys_msg = f"""You are an Explainable AI system for a Cyber-Physical System.
    ...
    **Recommended Action:** [1-sentence suggested repair or investigation step]
    """
    agent = AssistantAgent(name="Explainer_Bad", system_message=sys_msg, llm_config=llm_config)
    proxy = UserProxyAgent(name="Proxy_Explainer_Bad", human_input_mode="NEVER", max_consecutive_auto_reply=0, code_execution_config=False)
    msg = f"Please provide a context-agnostic explanation for this anomaly:\n\n{telemetry_prompt}"
    res = proxy.initiate_chat(agent, message=msg, clear_history=True)
    return res.summary

def run_explainer_aware(telemetry_prompt, config):
    sys_msg = f"""You are an Explainable AI system for a Cyber-Physical System.
    ...
    **Recommended Action:** [1-sentence actionable next step]
    """
    agent = AssistantAgent(name="Explainer_Good", system_message=sys_msg, llm_config=llm_config)
    proxy = UserProxyAgent(name="Proxy_Explainer_Good", human_input_mode="NEVER", max_consecutive_auto_reply=0, code_execution_config=False)
    msg = f"Please provide a context-aware explanation for this anomaly using all provided data:\n\n{telemetry_prompt}"
    res = proxy.initiate_chat(agent, message=msg, clear_history=True)
    return res.summary

def run_evaluator_agnostic(bad_exp, config):
    sys_msg = f"""You are a {config['expert_persona']}.
    ...
    **Actionability:** [Score]/5 - [1-sentence justification]
    """
    agent = autogen.AssistantAgent(name="Evaluator_Bad", system_message=sys_msg, llm_config=llm_config)
    proxy = autogen.UserProxyAgent(name="Proxy_Bad", human_input_mode="NEVER", max_consecutive_auto_reply=0, code_execution_config=False)
    msg = f"Please read the following explanation generated by an AI, and evaluate its Trust, Reasonableness, and Actionability as instructed.\n\n### EXPLANATION TO EVALUATE:\n{bad_exp}\n\n### INSTRUCTIONS:\nProvide the evaluation scores strictly in the requested format."
    res = proxy.initiate_chat(agent, message=msg)
    return res.summary

def run_evaluator_aware(good_exp, config):
    sys_msg = f"""You are a {config['expert_persona']}.
    ...
    **Actionability:** [Score]/5 - [1-sentence justification]
    """
    agent = autogen.AssistantAgent(name="Evaluator_Good", system_message=sys_msg, llm_config=llm_config)
    proxy = autogen.UserProxyAgent(name="Proxy_Good", human_input_mode="NEVER", max_consecutive_auto_reply=0, code_execution_config=False)
...
def parse_scores(eval_text):
    """Extract Trust / Reasonableness / Actionability numeric scores from eval response."""
    scores = {}
    for metric in ("Trust", "Reasonableness", "Actionability"):
        m = re.search(rf'\*\*{metric}:\*\*\s*(\d+\.?\d*)\s*/\s*5', eval_text)
        scores[metric] = float(m.group(1)) if m else None
    return scores
...
# ============================================================
# --- 7. BATCH EVALUATION ACROSS ALL ANOMALIES ---
# ============================================================
st.markdown('<hr class="section-divider">', unsafe_allow_html=True)
st.write("### 🧪 Batch Evaluation: Aggregate Scores Across Anomalies")
st.write(
    f"Run both agents on multiple detected anomalies, judge each, "
    f"then average Trust / Reasonableness / Actionability per agent. "
    f"**Total anomalies detected:** {len(anomalies)}"
)

total_anom = len(anomalies)
if total_anom == 0:
    st.warning("No anomalies detected in this dataset.")
else:
    max_n = st.slider(
        "Number of anomalies to process (more = slower, ~30s per anomaly on local Llama):",
        min_value=1,
        max_value=total_anom,
        value=min(5, total_anom),
    )

    if st.button(f"🧪 Run Batch Eval on {max_n} Anomalies"):
        progress_bar = st.progress(0.0)
        status_box = st.empty()
        rows = []
        sample = anomalies.head(max_n)

        for i, (idx, row_data) in enumerate(sample.iterrows()):
            status_box.info(f"Processing anomaly {i + 1}/{max_n} — Event ID {idx} @ {row_data['Timestamp']}")
            telemetry_prompt = build_telemetry_prompt(row_data, idx, row_data['Timestamp'], config)

            try:
                bad_exp = run_explainer_agnostic(telemetry_prompt, config)
                good_exp = run_explainer_aware(telemetry_prompt, config)
                bad_eval = run_evaluator_agnostic(bad_exp, config)
                good_eval = run_evaluator_aware(good_exp, config)
                bad_scores = parse_scores(bad_eval)
                good_scores = parse_scores(good_eval)
            except Exception as e:
                st.error(f"Failed on Event ID {idx}: {e}")
                continue

            rows.append({"event_id": idx, "timestamp": row_data['Timestamp'], "agent": "Context-Agnostic", **bad_scores})
            rows.append({"event_id": idx, "timestamp": row_data['Timestamp'], "agent": "Context-Aware", **good_scores})
            progress_bar.progress((i + 1) / max_n)

        status_box.success(f"✅ Batch evaluation complete. Processed {max_n} anomalies.")

        if not rows:
            st.error("No valid scored rows. Check eval format parsing.")
        else:
            df_scores = pd.DataFrame(rows)
            st.session_state['batch_scores'] = df_scores

            metric_cols = ['Trust', 'Reasonableness', 'Actionability']

            # Per-agent averages (drops NaNs from parse misses).
            agg = (
                df_scores.groupby('agent')[metric_cols]
                .mean()
                .round(2)
            )

            # Display: pivot into rows = metric, columns = agent.
            display_df = agg.T.rename_axis("Metric").reset_index()
            display_df = display_df[["Metric", "Context-Agnostic", "Context-Aware"]]
            display_df["Δ (Aware − Agnostic)"] = (
                display_df["Context-Aware"] - display_df["Context-Agnostic"]
            ).round(2)

            st.markdown("## 🏆 Batch Summary — Average Scores per Agent")
            st.dataframe(display_df, use_container_width=True, hide_index=True)

            # Overall winner.
            bad_overall = agg.loc["Context-Agnostic"].mean()
            good_overall = agg.loc["Context-Aware"].mean()
            if good_overall > bad_overall:
                winner_label = "Context-Aware (Proposed Framework) ✅"
                winner_score, loser_score = good_overall, bad_overall
            elif bad_overall > good_overall:
                winner_label = "Context-Agnostic (Traditional XAI) ❌"
                winner_score, loser_score = bad_overall, good_overall
            else:
                winner_label = "Tie"
                winner_score = loser_score = good_overall

            st.success(
                f"🏆 **Overall Winner:** {winner_label} — "
                f"avg {winner_score:.2f}/5 vs {loser_score:.2f}/5 "
                f"(across {max_n} anomalies × 3 metrics)"
            )

            # Per-metric winner breakdown.
            st.markdown("### Per-Metric Winner Breakdown")
            breakdown_rows = []
            for metric in metric_cols:
                b = agg.loc["Context-Agnostic", metric]
                g = agg.loc["Context-Aware", metric]
                if g > b:
                    w = "Context-Aware"
                elif b > g:
                    w = "Context-Agnostic"
                    ...
                    w = "Tie"
                breakdown_rows.append({"Metric": metric, "Context-Agnostic": b, "Context-Aware": g, "Winner": w})
            st.dataframe(pd.DataFrame(breakdown_rows), use_container_width=True, hide_index=True)

            # Raw per-anomaly scores expandable.
            with st.expander("📑 Per-anomaly raw scores"):
                st.dataframe(df_scores, use_container_width=True, hide_index=True)

            # Allow CSV download.
            st.download_button(
                "⬇️ Download batch scores (CSV)",
                df_scores.to_csv(index=False).encode("utf-8"),
                file_name=f"batch_scores_{config['system_label'].replace(' ', '_')}.csv",
                mime="text/csv",
            )